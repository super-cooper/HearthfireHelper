...
public class House {
	...
	private void determineExpenses() {
		for (Expense expense : Expense.values()) {
			if (expense != Expense.Chicken) {
				System.out.print("Do you want to buy a " + expense.name() 
						+ " for " + expense.cost() + " gold? (y/n) ");
				if (getUserInputPositive()) 
					expense.add(1);
			} else {
				System.out.print("How many chickens would you like to buy for 25 gold each?" 
						+ " (0-3) ");
				while (true) {
					int x = -1;
					try {
	...
	private void buildOutside() {
		...
		for (int i = 0; i < types.length; i++) {
			...
			System.out.print(BUILD_PROMPT + 
					(i == 0 ? "an " : "a ") + formatName(type.name()) + "? (y/n) ");
			if (getUserInputPositive()) {
				this.outside.addPiece(
						this.outside.new Furniture(type, 1));
			}
		}
	...
	private void buildFullManor() {
		...
		System.out.print("Do you want an arcane enchanter on the first floor" + 
				" of your main hall? (y/n) ");
		if(getUserInputPositive()) {
			mainHall.addPiece(
					mainHall.new Furniture(FurnitureType.Arcane_Enchanter, 1));
		}
		System.out.print("Do you want an alchemy lab on the first floor" + 
				" of your main hall? (y/n) ");
		if (getUserInputPositive()) {
			mainHall.addPiece(
					mainHall.new Furniture(FurnitureType.Alchemy_Lab, 1));
		}
	...
	private void buildTrophies(Room trophyRoom) {
		...
		String prompt = "Please pick " + largeTrophies + " large trophies."
				+ " (Separated by spaces)";

		for (int i = 0; i < largeOptions.length; i++) {
			FurnitureType large = largeOptions[i];
			System.out.println(i + ": " +
					large.name().replaceAll("Trophy_", "").
					replaceAll("_", " "));
		}
		...
		prompt = "Please pick " + smallTrophies + " small trophies."
				+ " (Separated by spaces)";

		for (int i = 0; i < smallOptions.length; i++) {
			FurnitureType small = smallOptions[i];
			System.out.println(i + ": " + small.name().replaceAll("Trophy_", "")
					.replaceAll("_", " "));
		}
	 ...
	 * @param options The trophies to pick from
	 * @param limit The limit on number of trophies
	 * @param picks 
	 */
	...
	private void buildCellar() {
		...
		System.out.print(BUILD_PROMPT + "smithing equipment in your cellar? " + 
				"(y/n) ");
		if (getUserInputPositive())
			this.rooms.add(new Room(RoomType.Cellar_Smithing));
		
		// build shrine
		System.out.print(BUILD_PROMPT + "shrines in your cellar?" +
				" (y/n) ");
		if (getUserInputPositive()) {
		...
		// build safes
		System.out.print(BUILD_PROMPT + "10 safes in your cellar? " +
				"(y/n) ");	
		if (getUserInputPositive())
	...
	private void buildShrines(Room base) {
		String[] divines = {"Akatosh", "Arkay", "Dibella", "Julianos", "Kynareth", 
		...
		for (String divine : divines) {
			...
			if (getUserInputPositive()) {
				Room.Furniture shrine = 
						Room.getFurnitureByName("Shrine_of_" + divine, base, 1);
	 ...
	 * with underscores
	 */
	public static String formatToEnum(String str) {
	...
	private boolean isHomestead() {
		return this.location == Location.Windstad_Manor ||
				this.location == Location.Heljarchen_Hall ||
				this.location == Location.Lakeview_Manor;
	}
	...
	private void assignOutsideFeature() {
		switch(this.location) {
			case Windstad_Manor:
				...
				if (getUserInputPositive())
					this.outside.addPiece 
						(Room.getFurnitureByName("Fish_Hatchery", outside, 1));
				break;
			case Lakeview_Manor:
				System.out.print(BUILD_PROMPT + "an apiary? (y/n) ");
				if (getUserInputPositive())
					this.outside.addPiece
						(Room.getFurnitureByName("Apiary", outside, 1));
				break;
			case Heljarchen_Hall:
				System.out.print(BUILD_PROMPT + "a grain mill? (y/n) ");
				if (getUserInputPositive())
					this.outside.addPiece
						(Room.getFurnitureByName("Grain_Mill", outside, 1));
				break;
			default: 
				break;
		}
	...
	private static boolean getUserInputPositive() {
		if (in.nextLine().trim().toLowerCase().startsWith("n"))
			return false;
		else
			return true;
	}
	...
	private HashMap<String, Integer> maxLength = new HashMap<String, Integer>();
	/**
	...
	private String formatString(String material, int amount, String category) {
		...
		sb.append(formatName(material) + ": ");
		// append appropriate spaces to keep all values aligned
		for (int i = 0; i < maxLength.get(category) - material.length(); i++)
			sb.append(' ');
		sb.append(amount);
	...
	/**
	 * Builds up max lengths for each category using recurrant calls
	...
	public String toString() { 
		...
		String tab = "    ";
		StringBuilder sb = new StringBuilder();
		HashMap<String, Integer> houseTotals = new HashMap<String, Integer>();
		
		this.buildMaxLengths();
		
		sb.append(formatName(this.location.name()) + ":\n");
		indent += tab;
		...
		for (Room room : this.rooms) {
			...
			sb.append(indent + formatName(val) + ":\n");
			indent += tab;
			
			// for all the furniture in each room in this house
			for (Room.Furniture piece : room.getPieces()) {
				sb.append(indent + formatName(piece.getName()) + ": " + piece.amount() + "\n");
				indent += tab;
				// for all the materials to build each furniture in each room in this hosue
				for (String material : piece.materials()) {
					sb.append(indent + formatString(material, 
							piece.getMaterialCount(material), 
							room.getName() + piece.getName())+ '\n');
				}
			...
			for (String material : roomMaterials.keySet()) {
				sb.append(indent + formatString(material, 
						roomMaterials.get(material),
						room.getName()) + '\n');
			}
			
			indent = indent.replaceFirst("\\s{4}+", "");
			sb.append(indent + "TOTALS:\n");
			indent += tab;
			...
			for (String material : totalMaterials.keySet()) {
				sb.append(indent + formatString(material, 
						totalMaterials.get(material), room.getName() + "total") 
						+ '\n');
		...
		sb.append(indent + "EXPENSES:\n");
		indent += tab;
		for (Expense expense : Expense.values()) {
			if (expense.count() <= 0)
				continue;
			sb.append(indent + formatString(expense.name(), expense.count(), 
					expense.getClass().getName()) + '\n');
			indent += tab;
			sb.append(indent + "Gold: " + expense.totalCost() + '\n');
			indent.replaceFirst(tab, "");
			MaterialCounter.tally("Gold", expense.totalCost(), houseTotals);
			indent = indent.replaceFirst("\\s{4}+", "");
		}
		sb.append(indent + formatName(location.name()) + 
				(this.isHomestead() ? " plot:\n" : " deed:\n"));
		indent += tab;
		sb.append(indent + "Gold: " + location.cost() + '\n');
		...
		for (String material : houseTotals.keySet()) {
			sb.append(indent + formatString(material, 
					houseTotals.get(material), location.name()) 
					+ '\n');
		}
	...
	private void buildMaxLengths() {
		...
		for (Expense expense : Expense.values()) {
			if (expense.count() > 0)
				this.buildMaxLength(expense.totalCost() + ' ' + expense.name(), 
					expense.getClass().getName());
		}
	}
	...
	private enum Expense {
		...
		public int totalCost() { return cost() * count(); }
	}
	...
	public static enum Location {
		...
		Location(int cost) { this.cost = cost; }
