...
public class Room {
	...
	//fields
	public static enum FurnitureType {Barrels, Chest, Cupboard, Desk,
	...
	public static enum RoomType {Small_House, Entryway, Main_Hall, Cellar,
	...
	private static final String ROOM_FMT_ERROR =
			"Improperly formatted room file \'" +
					ROOM_FILE + "\'!";
	...
	public Room(RoomType type) {
		this.type = type;
		this.pieces = new LinkedHashSet<Furniture>();
		this.roomMaterials = new HashMap<String, Integer>();
	 ...
	 */
	public LinkedHashSet<Furniture> getPieces() {
		return this.pieces;
	}
	...
	private void buildProperties() {
		Scanner scan = null;
		try {
			scan = new Scanner(new File(getResourceDirectory() + ROOM_FILE));
		} catch (FileNotFoundException e) {
			System.err.println("Missing room info file \'" +
					ROOM_FILE + "\'!");
			System.exit(1);
		}
		...
		while (scan.hasNextLine()) {
			...
			if (line.startsWith(ROOM_TYPE_NOTATOR)) {
				...
				if (line.trim().replaceFirst(ROOM_TYPE_NOTATOR, "")
						.equals(this.getName())) {
					...
					while (scan.hasNextLine() &&
							!line.startsWith(ROOM_TYPE_NOTATOR)) {

						if (line.contains(Furniture.FURNITURE_TYPE_NOTATOR)) {

							String[] tokens = line.trim()
									.replaceFirst(Furniture.FURNITURE_TYPE_NOTATOR, " ")
										.split(" ");
							...
							this.addPiece(new Furniture(
									FurnitureType.valueOf(tokens[0]),
							...
							if (tokens.length >= 2)
								amt = tokens[1];
							try {
								this.roomMaterials.put(tokens[0],
										Integer.parseInt(amt));
		...
		System.err.println("Give room type \'" + this.getName() +
				"\' not found in room file \'" + ROOM_FILE + "\'!");
		System.exit(1);
	}
	...
	private static String getResourceDirectory() {
		return System.getProperty("user.dir") + "\\info\\";
	}
	 ...
	 */
	public static String[] fixSplit(String[] badArray) {
		int newLen = 0;
	 ...
	 */
	public HashMap<String, Integer> getTotalMaterials() {
	 ...
	 * @param amount The amount of the new furniture to add
	 * @return A new furniture object of the requested type, or null if type DNE
	 */
	public static Furniture getFurnitureByName(String furnitureName, Room room, int amount) {
		for (FurnitureType item : FurnitureType.values())
			if (item.name().equals(furnitureName)) {
				return room.new Furniture(item, amount);
			}
		return null;
	}
	 ...
	 */
	public void addPiece(Furniture piece) {
	...
	public class Furniture {
		 ...
		 */
		public Furniture(FurnitureType type, int amount) throws IllegalArgumentException {
			/*if (containsFurnitureType(type))
				throw new IllegalArgumentException("Cannot have two instances"
						+ " of the same furniture type.");*/
		...
		private void buildMaterialsList() {
			Scanner scan = null;
			try {
				scan = new Scanner(new File(getResourceDirectory() + FURNITURE_FILE));
			} catch (FileNotFoundException e) {
				System.err.println("Furniture material file \"" +
						FURNITURE_FILE + "\" missing!");
				System.exit(1);
			}
			...
			while (scan.hasNextLine()) {
				...
				if (line.contains(FURNITURE_TYPE_NOTATOR)) {
					...
					if (line.replaceAll(FURNITURE_TYPE_NOTATOR, "").trim()
							.equals(this.type.name())) {

						line = scan.nextLine();

						// gather materials below
						while (scan.hasNextLine() &&
								!line.contains(FURNITURE_TYPE_NOTATOR)) {
			...
			System.err.println("Furniture type " + this.getName() +
					" not found in file \'" + FURNITURE_FILE + "\'!");
			System.exit(1);
		}
		 ...
		 */
		public int getTotalMaterialCount(String material) {
